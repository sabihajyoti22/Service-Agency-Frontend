<template>
  <div class="default-layout py-5">
    <div
      id="feedback"
      class="text-center text-primary text-3xl font-extrabold mb-3"
    >
      Clients Feed <span class="text-navy">Back</span>
    </div>

    <div class="h-56 md:h-fit overflow-y-scroll">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <fieldset
          v-for="(item, index) in reviews"
          :key="index"
          class="border border-navy p-4 rounded-lg"
        >
          <legend>
          ...
          <div class="text-sm line-clamp-3 mb-5">
            {{ item.comment }}
          </div>
          <div>Provided By:</div>
          <div class="text-sm">
  ...
  export default {
    ...
    created(){
      this.$axios.get("/reviews")
        .then((res) => {
          this.reviews = JSON.parse(JSON.stringify(res.data))
        })
    }
  }
</script>